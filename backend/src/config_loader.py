...
# Note: rpc_endpoint OR rpc_endpoints is required (handled in validate_config)
...
def validate_config(config: dict) -> None:
    ...
    # Validate RPC endpoint(s) - support both single and multiple
    if "rpc_endpoint" not in config and "rpc_endpoints" not in config:
        raise ValueError("Either 'rpc_endpoint' or 'rpc_endpoints' must be specified")
    
    # Normalize: if rpc_endpoints is provided, ensure it's a list
    # If rpc_endpoint is provided, keep it as is (backward compatible)
    if "rpc_endpoints" in config:
        if not isinstance(config["rpc_endpoints"], list):
            raise ValueError("'rpc_endpoints' must be a list of endpoint URLs")
        if len(config["rpc_endpoints"]) == 0:
            raise ValueError("'rpc_endpoints' must contain at least one endpoint")
...
def print_config_summary(config: dict) -> None:
    ...
    logger.info("Bot name: %s", config.get("name", "unnamed"))
    logger.info("Platform: %s", platform_str)
    logger.info(
        "Listener type: %s",
        config.get("filters", {}).get("listener_type", "not configured"),
    )
    ...
    try:
        ...
        if listener_type and not validate_platform_listener_combination(
            ...
            logger.warning(
                "Listener '%s' may not be compatible with platform '%s'",
                listener_type,
                platform_str,
            )
    except ValueError:
        logger.warning("Invalid platform '%s'", platform_str)

    trade = config.get("trade", {})
    logger.info("Trade settings:")
    logger.info("  - Buy amount: %s SOL", trade.get("buy_amount", "not configured"))
    logger.info(
        "  - Buy slippage: %s%%",
        trade.get("buy_slippage", "not configured") * 100,
    )
    logger.info(
        "  - Extreme fast mode: %s",
        "enabled" if trade.get("extreme_fast_mode") else "disabled",
    )

    fees = config.get("priority_fees", {})
    logger.info("Priority fees:")
    if fees.get("enable_dynamic"):
        logger.info("  - Dynamic fees enabled")
    elif fees.get("enable_fixed"):
        logger.info(
            "  - Fixed fee: %s microlamports",
            fees.get("fixed_amount", "not configured"),
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        try:
            ...
            logger.info(
                "Supported listeners for this platform: %s",
                get_supported_listeners_for_platform(platform),
            )
        except Exception as e:
        ...
        logger.info("Valid configs: %s", len(results["valid_configs"]))
        logger.info("Invalid configs: %s", len(results["invalid_configs"]))
        logger.info("Platform distribution: %s", results["platform_distribution"])
        logger.info("Listener distribution: %s", results["listener_distribution"])

        if results["invalid_configs"]:
            logger.info("Invalid configurations:")
            for invalid in results["invalid_configs"]:
                logger.info("  %s: %s", invalid["file"], invalid["error"])
