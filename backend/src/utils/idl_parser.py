...
class IDLParser:
    ...
    def validate_instruction_data_length(
        ...
        if actual_size < expected_min_size:
            ...
            if self.verbose:
                logger.warning(
                    "Instruction data for '%s' shorter than expected (%s/%s bytes)",
                    instruction_name,
                    actual_size,
                    expected_min_size,
                )
    ...
    def decode_instruction(
        ...
        for arg in instruction.get("args", []):
            try:
                ...
                if self.verbose:
                    logger.error("Decode error in argument '%s': %s", arg["name"], e)
                return None
    ...
    def decode_event_data(
        ...
        if discriminator not in self.events:
            if self.verbose:
                logger.debug("Unknown event discriminator: %s", discriminator.hex())
            return None
        ...
        if event_name and event_def["name"] != event_name:
            if self.verbose:
                logger.debug(
                    "Event name mismatch: expected %s, got %s",
                    event_name,
                    event_def["name"],
                )
            return None
        ...
        if event_name_actual not in self.types:
            if self.verbose:
                logger.debug("Event type %s not found in types section", event_name_actual)
            return None
        ...
        try:
            ...
            if event_type.get("kind") != "struct":
                if self.verbose:
                    logger.debug(
                        "Event %s is not a struct type: %s",
                        event_name_actual,
                        event_type.get("kind", "NO KIND"),
                    )
                    logger.debug(
                        "Available keys in type_def: %s", list(type_def.keys())
                    )
                    logger.debug("Event type structure: %s", event_type)
                return None
            ...
            if self.verbose:
                logger.debug(
                    "Decoding %s fields for event %s", len(fields), event_name_actual
                )

            for field in fields:
                if self.verbose:
                    logger.debug(
                        "Decoding field: %s (%s)", field["name"], field["type"]
                    )
                ...
                try:
                    ...
                    if self.verbose:
                        logger.debug("Error decoding field %s: %s", field["name"], e)
            ...
            if self.verbose:
                logger.error("Error decoding event %s: %s", event_name_actual, e)
            return None
        ...
        for log in logs:
            if "Program data:" in log:
                try:
                    ...
                    if self.verbose:
                        logger.debug("Failed to decode log data: %s", e)
                    continue
    ...
    def decode_account_data(
        ...
        try:
            if account_type_name not in self.types:
                if self.verbose:
                    logger.debug("Account type '%s' not found in IDL", account_type_name)
                return None
            ...
            if skip_discriminator:
                if len(account_data) < DISCRIMINATOR_SIZE:
                    if self.verbose:
                        logger.debug(
                            "Account data too short to contain a discriminator: %s bytes",
                            len(account_data),
                        )
                    return None
            ...
            if self.verbose:
                logger.error(
                    "Error decoding account data for %s: %s", account_type_name, e
                )
            return None
    ...
    def _build_event_map(self):
        ...
        for event in self.idl.get("events", []):
            ...
            if self.verbose:
                logger.debug(
                    "Loaded event: %s with discriminator %s",
                    event["name"],
                    discriminator.hex(),
                )
        ...
        for discriminator, instruction in self.instructions.items():
            try:
                ...
                if self.verbose and instruction["name"] == "initialize":
                    logger.debug(
                        "Initialize instruction min size: %s bytes", min_size
                    )
            except Exception as e:
                if self.verbose:
                    logger.warning(
                        "Could not calculate size for %s: %s",
                        instruction["name"],
                        e,
                    )
