...
async def start_bot(config_path: str):
    ...
    try:
        # Support both single endpoint (backward compatible) and multiple endpoints (failover)
        rpc_endpoint = cfg.get("rpc_endpoints", cfg.get("rpc_endpoint"))
        if rpc_endpoint is None:
            raise ValueError("Either 'rpc_endpoint' or 'rpc_endpoints' must be specified in config")
        ...
        trader = UniversalTrader(
            ...
            enable_security_scan=cfg.get("security", {}).get("enable_scan", True),
            min_security_score=cfg.get("security", {}).get("min_score", 40.0),
            block_honeypots=cfg.get("security", {}).get("block_honeypots", True),
            block_critical_risk=cfg.get("security", {}).get("block_critical_risk", True),
            security_cache_ttl=cfg.get("security", {}).get("cache_ttl", 300),
        )
